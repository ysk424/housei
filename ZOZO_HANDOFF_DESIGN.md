...
Housei can still say why a garment will not solve, in Housei's own words, on
Housei's own vocabulary. Once the mesh is inside the ZOZO Contact Solver, the
only thing left to report is what the solver saw, and that turns out to be
much less than what Housei knew.

The button runs four stages, in this order:

1. **Re-cut** — `remesh_with_seam_counts` on the collection.
2. **Copy** — cloth and Body into a ZOZO-owned collection; Housei's own
...
"Current" is judged two ways, and the order between them is load-bearing.
The triangulation scheme, recorded on every part as `housei_cut_scheme`, is
checked first: scheme 3 is the uniform equilateral triangular lattice with no
seam paving band (see `SEAM_BOUNDARY_LAYER_DESIGN.md` for what scheme 1 was
and the measurements that retired it), and a part carrying an older scheme —
or none — is re-cut clean at its natural counts before anything else looks at
it. Only then are seam counts measured and matched. Scheme 2 was the same
lattice with those two steps in the other order, and it lasted a day: the
count targets were measured on the stale meshes about to be replaced, so a
side seam the old doubling bug had blown up to 16x its natural density became
the target the clean side was forced up to, and the solve showed a fan of
seventeen stitches on every vertex of the short side. The seam-count pass
itself is convergent now — one adapt matches both sides of every seam 1:1 and
a second adapt is a no-op — where the per-segment forced counts it replaced
doubled the side seams on every press.
...
topology is what Housei holds by index: seam pairs, pattern coordinates,
...
An animated collider is captured per frame and driven kinematically (ordinary
for ZOZO). MCP also turns on **Apply Soft Constraints**
(`enable_soft_constraint=True`, stiffness `10.0`) so contact can yield the
body — e.g. a closing armpit — instead of trapping cloth. Soft constraints
promote the body into a simulated spring-held collider, which costs more than
a hard pin. Nothing in stage 4 mesh-checks the body surface itself.
