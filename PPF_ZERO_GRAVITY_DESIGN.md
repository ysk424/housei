...
Zero GRAVITY closes every seam of a garment. It is the one step where Housei's
...
square-lattice solver is gone: sewing finishes inside Housei rather than being
exported, so the ZOZO hand-off no longer opens, welds or clears anything and
passes the garment on as it stands.

## Why it is affordable

Two promises Housei can make about its own state, both of which the hand-off
gave up.
...
The obvious answer is to stop replacing the mesh and hand over Housei's own.
...
| --- | ---: | ---: | ---: | ---: |
| Housei's own triangulation | 3.75e-12 m² | 0.12 um | — | — |
...
are, not which edges join them. Housei's own point set cannot be triangulated
...
reported in Housei before anything is transferred — but the gate sits four
