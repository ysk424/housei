...
Housei's own objects are never touched -- ZOZO gets copies, in a collection
of its own.
"""
...
class ZozoHandoffError(RuntimeError):
    """The current Housei state cannot safely be handed to ZOZO."""
    ...
    def mcp_configuration(self, scene: bpy.types.Scene) -> dict:
        ...
        return {
            ...
            "body_properties": {
                "contact_gap": ZOZO_CONTACT_GAP_M,
                "contact_offset": 0.0,
                # Soft pins on the STATIC body so contact can yield the
                # collider (armpit closing, limb against torso) instead of
                # trapping cloth with nowhere to go. Requires ZOZO add-on
                # that exposes enable_soft_constraint (1.0.16+).
...
def _handoff_collection(context, source: bpy.types.Collection) -> bpy.types.Collection:
    matches = [
        ...
        if collection.get("housei_role") == _HANDOFF_COLLECTION_ROLE
        and collection.get("housei_source_collection") == source.name
    ]
    ...
    handoff["housei_role"] = _HANDOFF_COLLECTION_ROLE
    handoff["housei_source_collection"] = source.name
    for collection in matches:
        for obj in list(collection.objects):
            if (
                obj.get("housei_source_collection") == source.name
                and obj.get("housei_role") in {_HANDOFF_CLOTH_ROLE, _HANDOFF_BODY_ROLE}
            ):
...
def _create_cloth_object(
    ...
    for part_index, obj in enumerate(parts):
        ...
        panel_index = int(obj.get("housei_panel_index", part_index))
    ...
    try:
        ...
        for item, value in zip(panel_attribute.data, face_panel_indices):
            item.value = value
        part_attribute = mesh.attributes.new(name="housei_source_part", type="INT", domain="POINT")
        for item, value in zip(part_attribute.data, vertex_part_indices):
            item.value = value
        stitch_attribute = mesh.attributes.new(name="housei_zozo_stitch", type="BOOLEAN", domain="EDGE")
        found_stitches = 0
        for edge in mesh.edges:
        ...
        uv_layer = mesh.uv_layers.new(name="Housei Pattern", do_init=False)
        ...
        cloth["housei_schema"] = "housei-pattern/1.0.0"
        cloth["housei_role"] = _HANDOFF_CLOTH_ROLE
        cloth["housei_source_collection"] = source.name
        cloth["housei_source_parts"] = [part.name for part in parts]
        cloth["housei_zozo_contact_gap_m"] = ZOZO_CONTACT_GAP_M
        return cloth
    except Exception:
...
def _create_body_object(
    ...
    *,
    ...
    try:
        ...
        pass
    duplicate["housei_role"] = _HANDOFF_BODY_ROLE
    duplicate["housei_source_collection"] = source.name
    duplicate["housei_source_body"] = body.name
    if z_min_m is not None and z_max_m is not None:
        ...
        duplicate["housei_body_z_min_m"] = float(min(z_min_m, z_max_m))
        duplicate["housei_body_z_max_m"] = float(max(z_min_m, z_max_m))
        duplicate["housei_body_faces_kept"] = int(kept)
        duplicate["housei_body_faces_total"] = int(total)
...
def _cloth_quality(
    ...
    attribute = mesh.attributes.get("housei_source_part")
...
def _project_name(collection_name: str) -> str:
    ...
    return f"housei_{value or 'clothes'}"
...
def prepare_for_zozo(
    ...
    *,
    ...
    """Create solver-owned cloth/body copies and leave Housei untouched.
    ...
    # The same pass replaces panels cut by an older triangulation scheme
    # (`housei_cut_scheme`): a panel still carrying the seam-band mesh is
    # re-cut onto the uniform triangular lattice, so what ZOZO simulates has
    # no seam-shaped topology for the drape to trace.
    try:
    ...
    cloth_group_name = f"Housei {collection.name} Cloth"
    body_group_name = f"Housei {collection.name} Body"
    cloth["housei_zozo_group"] = cloth_group_name
    body_copy["housei_zozo_group"] = body_group_name
    ...
    cloth["housei_shell_isect"] = shell_report.summary()
    cloth["housei_shell_isect_version"] = shell_report.version
    cloth["housei_shell_isect_include_body"] = bool(shell_report.include_body)
    cloth["housei_shell_isect_pipeline"] = shell_report.pipeline_token()
    cloth["housei_shell_isect_checks_run"] = int(shell_report.checks_run)
    cloth["housei_shell_isect_fix_attempted"] = bool(shell_report.fix_attempted)
    cloth["housei_shell_isect_pairs_before"] = int(shell_report.pairs_before)
    cloth["housei_shell_isect_pairs_after"] = int(shell_report.pairs_after)
    cloth["housei_shell_isect_fix"] = shell_report.fix_status
    cloth["housei_shell_isect_cloth_faces"] = int(shell_report.n_cloth_faces)
    if shell_report.pairs:
        cloth["housei_shell_isect_face_pairs"] = [
            f"{a},{b}" for a, b in shell_report.pairs
        ]
    elif "housei_shell_isect_face_pairs" in cloth:
        del cloth["housei_shell_isect_face_pairs"]
    ...
    cloth["housei_rest_area_min_m2"] = float(quality.area_min_m2)
    cloth["housei_rest_area_floor_m2"] = float(quality.area_floor_m2)
    cloth["housei_edge_min_m"] = float(quality.edge_min_m)
    cloth["housei_aspect_min"] = float(quality.aspect_min)
    cloth["housei_quality_failing_faces"] = int(quality.failing_faces)
